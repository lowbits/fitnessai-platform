...
class CreateMealPlanPrompt implements Stringable
{
     ...
     * generated this week with their three variety axes — primary_protein,
     * format, hero_veg. The new meal MUST NOT match ALL THREE of those axes
     * against any prior meal — i.e. at least one of {protein, format, hero_veg}
     * must differ. That's the "no template-twins" floor: e.g. a tofu-broccoli
     * noodle bowl and a tofu-broccoli soba bowl share all three and are blocked.
     *
    ...
    private function buildSlotConstraints(): string
    {
        ...
        foreach ($this->slotPlan as $slot => $plan) {
            ...
            $label = ucfirst($slot);
            $lines = ["Prior {$label} meals this week — your new {$label} MUST differ on at least ONE of {primary_protein, format, hero_veg} from every meal listed (matching all three on any one of them is forbidden):"];

            foreach ($forbidden as $meal) {
                $protein = $meal->primary_protein ?: '?';
                $format = $meal->format ?: '?';
                $heroVeg = $meal->hero_veg ?: '?';
                $lines[] = "- \"{$meal->name}\" [protein: {$protein}, format: {$format}, hero_veg: {$heroVeg}]";
            }
